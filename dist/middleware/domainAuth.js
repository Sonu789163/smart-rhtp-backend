...
const domainAuthMiddleware = async (req, res, next) => {
    try {
        ...
        const requestedWorkspace = req.headers["x-workspace"] || req.user.currentWorkspace;
        // Get user with workspace access information
        const user = await User_1.User.findById(req.user._id).select("domain accessibleWorkspaces currentWorkspace");
        if (!user) {
        ...
        const hasPrimaryDomainAccess = user.accessibleWorkspaces.some((ws) => (ws.workspaceDomain || "").toLowerCase() ===
        ...
        const hasChanges = !user.currentWorkspace ||
        ...
        if (requestedWorkspace) {
            ...
            const hasAccess = user.accessibleWorkspaces.some((ws) => ws.workspaceDomain === requestedWorkspace && ws.isActive);
            if (hasAccess) {
