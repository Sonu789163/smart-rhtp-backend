...
exports.workspaceRequestController = void 0;
const WorkspaceRequest_1 = require("../models/WorkspaceRequest");
const WorkspaceMembership_1 = require("../models/WorkspaceMembership");
const Workspace_1 = require("../models/Workspace");
const User_1 = require("../models/User");
exports.workspaceRequestController = {
    ...
    async requestAccess(req, res) {
        try {
            ...
            const existingRequest = await WorkspaceRequest_1.WorkspaceRequest.findOne({
                userId: user._id,
                workspaceId,
                status: "pending",
            });
            ...
            // Create request
            const request = new WorkspaceRequest_1.WorkspaceRequest({
                userId: user._id,
                workspaceId,
                domainId: userWithDomain.domainId,
                status: "pending",
    ...
    async getPendingRequests(req, res) {
        try {
            ...
            const requests = await WorkspaceRequest_1.WorkspaceRequest.find({
                workspaceId,
                domainId: userWithDomain.domainId,
                status: "pending",
            })
    ...
    async reviewRequest(req, res) {
        try {
            ...
            // Get request
            const request = await WorkspaceRequest_1.WorkspaceRequest.findById(requestId);
            if (!request) {
                return res.status(404).json({ message: "Request not found" });
            }
            // Get user's domainId to verify workspace belongs to domain
            const userWithDomain = await User_1.User.findById(user._id).select("domainId");
            if (!(userWithDomain === null || userWithDomain === void 0 ? void 0 : userWithDomain.domainId) || request.domainId !== userWithDomain.domainId) {
    ...
    async getMyRequests(req, res) {
        try {
            ...
            const requestsWithWorkspace = await Promise.all(requests.map(async (req) => {
                const workspace = await Workspace_1.Workspace.findOne({ workspaceId: req.workspaceId });
                return {
                    ...req.toObject(),
    ...
    async getAvailableWorkspaces(req, res) {
        try {
            const user = req.user;
            if (!user)
                return res.status(401).json({ message: "Unauthorized" });
            ...
            const allWorkspaces = await Workspace_1.Workspace.find({
                domainId: userWithDomain.domainId,
                status: "active",
            });
            ...
            const pendingRequests = await WorkspaceRequest_1.WorkspaceRequest.find({
                userId: user._id,
                status: "pending",
            });
