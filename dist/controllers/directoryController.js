...
exports.directoryController = {
    ...
    async create(req, res) {
        ...
        try {
            ...
            const { normalizeCompanyName, findSimilarDirectories } = await Promise.resolve().then(() => __importStar(require("../lib/companyNameNormalizer")));
            const normalizedName = normalizeCompanyName(trimmedName);
            // Check for exact match (normalized name)
            const existingExact = await Directory_1.Directory.findOne({
                workspaceId: currentWorkspace,
        ...
        try {
            ...
            const visibleDirs = await Promise.all(allDirs.map(async (dir) => {
                ...
                if (isCrossDomainUser) {
                    ...
                    const userEmail = (_b = (_a = req.user) === null || _a === void 0 ? void 0 : _a.email) === null || _b === void 0 ? void 0 : _b.toLowerCase();
                    if (userId || userEmail) {
                        ...
                        let userShare = userId ? await SharePermission_1.SharePermission.findOne({
                        ...
                        if (userShare) {
                            ...
                            const anyShareForUser = userId ? await SharePermission_1.SharePermission.findOne({
                                scope: "user",
                                principalId: userId,
                                resourceType: "directory",
                            }) : null;
                            const anyShareForEmail = userEmail ? await SharePermission_1.SharePermission.findOne({
                                scope: "user",
                                invitedEmail: userEmail,
                                resourceType: "directory",
                            }) : null;
                            if (anyShareForUser || anyShareForEmail) {
                                console.log(`[listChildren] ✗ SharePermission exists for user but NOT for directory ${dir.name} (${dir.id}). User has SharePermission for: ${(anyShareForUser === null || anyShareForUser === void 0 ? void 0 : anyShareForUser.resourceId) || (anyShareForEmail === null || anyShareForEmail === void 0 ? void 0 : anyShareForEmail.resourceId)}`);
                            }
                            else {
            ...
            if (userId && parentId === null) { // Only for root level
                try {
                    const pendingShares = await SharePermission_1.SharePermission.find({
                        scope: "user",
                        resourceType: "directory",
                        $or: [
                            { principalId: userId },
                            { invitedEmail: (_h = (_g = req.user) === null || _g === void 0 ? void 0 : _g.email) === null || _h === void 0 ? void 0 : _h.toLowerCase() }
                        ]
                    });
                    for (const share of pendingShares) {
                        ...
                        if (!existingSharedDir) {
                            ...
                            if (originalDirectory) {
                                // Get user's domain
                                const userWithDomain = await User_1.User.findById(userId).select("domainId domain");
                                if (userWithDomain) {
            ...
            if (isCrossDomainUser && userId && dirs.length === 0) {
                ...
                if (allUserShares.length === 0) {
                    ...
                    const invitation = await WorkspaceInvitation_1.WorkspaceInvitation.findOne({
                        inviteeEmail: (_k = (_j = req.user) === null || _j === void 0 ? void 0 : _j.email) === null || _k === void 0 ? void 0 : _k.toLowerCase(),
                        workspaceId: currentWorkspace,
                        status: "accepted",
                    });
                                        ...
                                        if (!existingShare) {
                                            ...
                                            try {
                                                ...
                                                if (upsertError.code === 11000) {
                                                    ...
                                                    if (((_l = upsertError.keyPattern) === null || _l === void 0 ? void 0 : _l.scope) === 1 && ((_m = upsertError.keyPattern) === null || _m === void 0 ? void 0 : _m.linkToken) === 1) {
            ...
            if (parentId) {
                const parentDirectory = await Directory_1.Directory.findOne({ id: parentId, workspaceId: currentWorkspace });
                if ((parentDirectory === null || parentDirectory === void 0 ? void 0 : parentDirectory.isShared) && (parentDirectory === null || parentDirectory === void 0 ? void 0 : parentDirectory.sharedFromDirectoryId)) {
            ...
            if (isViewingSharedDirectory && originalDirectoryId) {
                ...
                const originalDir = await Directory_1.Directory.findOne({ id: originalDirectoryId });
                if (originalDir) {
            ...
            const docFilter = docFilterConditions.length > 1 ? { $or: docFilterConditions } : docFilterConditions[0];
            // Sorting
    ...
    async search(req, res) {
        try {
            ...
            const textMatches = await Directory_1.Directory.find({
                workspaceId: currentWorkspace,
                parentId: null,
                $or: [
    ...
    async checkDuplicate(req, res) {
        try {
            ...
            const exactMatch = await Directory_1.Directory.findOne({
                workspaceId: currentWorkspace,
