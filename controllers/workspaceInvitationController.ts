...
export const workspaceInvitationController = {
  ...
  async sendInvitation(req: AuthRequest, res: Response) {
    try {
      ...
      if (existingUser) {
        const hasAccess = existingUser.accessibleWorkspaces.some(
          (ws: any) => ws.workspaceDomain === workspaceDomain && ws.isActive
        );
        if (hasAccess) {
          return res.status(400).json({
  ...
  async acceptInvitation(req: AuthRequest, res: Response) {
    try {
      ...
      user.accessibleWorkspaces.push({
        ...
        allowedTimeBuckets: invitation.allowedTimeBuckets?.length
  ...
  async getUserWorkspaces(req: AuthRequest, res: Response) {
    try {
      const user = await User.findById(req.user._id).select(
        "accessibleWorkspaces currentWorkspace"
      );
      ...
      res.json({
        workspaces: user.accessibleWorkspaces.filter((ws: any) => ws.isActive),
